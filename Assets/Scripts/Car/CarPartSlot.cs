...
public class CarPartSlot : MonoBehaviour, IInteractable
{
    ...
    private void EnsurePartVisualHasCollider()
    {
        if (partVisual == null) return;
        
        Collider[] cols = partVisual.GetComponentsInChildren<Collider>(true);
        ...
        if (!hasValidCollider)
        {
            ...
            Bounds localBounds = new Bounds(partVisual.transform.InverseTransformPoint(visualRenderers[0].bounds.center), Vector3.zero);
            ...
            foreach (Renderer r in visualRenderers)
            {
                ...
                Vector3[] corners = new Vector3[8];
                corners[0] = partVisual.transform.InverseTransformPoint(c + new Vector3(ext.x, ext.y, ext.z));
                corners[1] = partVisual.transform.InverseTransformPoint(c + new Vector3(ext.x, ext.y, -ext.z));
                corners[2] = partVisual.transform.InverseTransformPoint(c + new Vector3(ext.x, -ext.y, ext.z));
                corners[3] = partVisual.transform.InverseTransformPoint(c + new Vector3(ext.x, -ext.y, -ext.z));
                corners[4] = partVisual.transform.InverseTransformPoint(c + new Vector3(-ext.x, ext.y, ext.z));
                corners[5] = partVisual.transform.InverseTransformPoint(c + new Vector3(-ext.x, ext.y, -ext.z));
                corners[6] = partVisual.transform.InverseTransformPoint(c + new Vector3(-ext.x, -ext.y, ext.z));
                corners[7] = partVisual.transform.InverseTransformPoint(c + new Vector3(-ext.x, -ext.y, -ext.z));

                foreach (Vector3 corner in corners)
                    localBounds.Encapsulate(corner);
            }

            BoxCollider bc = partVisual.AddComponent<BoxCollider>();
